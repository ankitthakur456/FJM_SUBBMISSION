    ...
    while True:
        try:
            ...
            if data:
                ...
                if FL_machine_status and time.time() - prev_time <= 29:
                    GL_MIN_PRESSURE = data.get('testingPressure') if data.get(
                        'testingPressure') < GL_MIN_PRESSURE else GL_MIN_PRESSURE
                    FL_STATUS = True
                else:
                    FL_STATUS = False

                # if FL_machine_status:
                #     FL_STATUS = True
                #     if data.get('testingPressure') > GL_MAX_PRESSURE:
                #         GL_MAX_PRESSURE = data.get('testingPressure')
                # else:
                #     FL_STATUS = False
...
#             pressure_data = get_masibus_data()
#             log.info(f"[+] \t\t\tpressure_sensor {pressure_data}")
#         except Exception as e:
