...
def read_values(mb_client, parameters):
    ...
    try:
        ...
        data0 = f_list(mb_client.read_holding_registers(22, 6), False)  # here we are reading temperature 1 and 2 values and energy
    ...
    while True:
        try:
            ...
            if data:
                ...
                print(f"{heating_time}  > {GL_MAX_HEATING_TIME}")
                if heating_time > GL_MAX_HEATING_TIME:
                    GL_MAX_HEATING_TIME = heating_time
                    print(f"{new_temp} > {GL_MAX_TEMP}")
