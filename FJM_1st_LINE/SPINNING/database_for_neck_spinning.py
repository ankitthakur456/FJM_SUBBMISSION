...
log = logging.getLogger("ESCORTS LOGS")
...
class DBHelper:
    def __init__(self):
        ...
        self.cursor.execute("""CREATE TABLE IF NOT EXISTS running_data(
            timestamp INTEGER, induction_temp REAL,
            o2_pressure_heat REAL,
            o2_pressure_cutting REAL,
            png_pressure REAL,
            propane_pressure REAL,
            daacytylene_pressure REAL,
            hydraulic_power_pack REAL,
            forming_temp REAL,
    ...
    def save_running_data(self, induction_temp, o2_pressure_heat, o2_pressure_cut, png_pressure,
                          propane_pressure, daccytelene_pressure, hydraulic_power_pack, forming_temp, serial_number):
        try:
            ...
            if data:
                ...
                timestamp=?, induction_temp=?, o2_pressure_heat=?, o2_pressure_cutting=?,
                png_pressure=?, propane_pressure=?, daacytylene_pressure=?, hydraulic_power_pack=?, forming_temp=?,
                where serial_number =?""",
                                    (time.time(), induction_temp, o2_pressure_heat, o2_pressure_cut, png_pressure,
                                     propane_pressure, daccytelene_pressure, hydraulic_power_pack, forming_temp,
                                     serial_number))
            else:
                self.cursor.execute("""INSERT INTO running_data(timestamp,
                induction_temp, o2_pressure_heat, o2_pressure_cutting,
                png_pressure, propane_pressure, daacytylene_pressure, hydraulic_power_pack, forming_temp, serial_number)
                VALUES(?,?,?,?,?,?,?,?,?,?)""",
                                    (time.time(), induction_temp, o2_pressure_heat, o2_pressure_cut, png_pressure,
                                     propane_pressure, daccytelene_pressure, hydraulic_power_pack, forming_temp,
                                     serial_number))
