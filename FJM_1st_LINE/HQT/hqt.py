...
def initiate(client):
    try:
        client.connect("192.168.0.1", 0,
                       0)  # 2 for s7-300 # first 0 for port number(0 arrg  automatically assign an available port), second 0 for timeout value.
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # GL_NEW_QUENCHING_TIME = GL_QUENCHING_TIME
            # log.info(f"New Quenching Time: {GL_NEW_QUENCHING_TIME}")
            ...
            if data:
                ...
                #     GL_PREV_CYCLE_TIME = cycle_time
                #     FL_FIRST_CYCLE_RUN = False

                # if cycle_time > FL_FIRST_CYCLE_RUN:
                #     FL_STATUS = True
                # else:
                #     FL_STATUS = False
                # FL_STATUS = get_status()['status']

                # if GL_NEW_QUENCHING_TIME == 0:
                #     FL_STATUS = True
                # elif GL_NEW_QUENCHING_TIME == 200:
                #     FL_STATUS = False
                ...
                if quenching_time <= 120:
                    quench_mode = 1
                    FL_machine_status = get_machine_status(FL_machine_status)

                # else:
                #     quench_mode = None
                if double_quenching_time <= 120:
                    quench_mode = 2
                    FL_machine_status = get_machine_status(FL_machine_status)
                # else:
                #     quench_mode = None
                ...
                # if 0 <= quenching_time < 100:
                #     FL_STATUS = True
                #     GL_DOOR_CLOSE_TIME = datetime.datetime.now() + datetime.timedelta(seconds=200)
                # elif quenching_time >= 200:
                #     FL_STATUS = False
                #     GL_DOOR_CLOSE_TIME = datetime.datetime.now()
                ...
                # log.info(f"{FL_STATUS} {FL_PREV_STATUS}")
            else:
