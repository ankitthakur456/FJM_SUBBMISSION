    ...
    while True:
        try:
            if GL_SERIAL_NUMBER:
                ...
                log.info(f"[+] Enqueueing serial number to db {GL_SERIAL_NUMBER}")
                values = {
            ...
            ob_db.delete_serial_number('null1')
            log.info(f"[+] Data is {data}")
            if data:
                ...
                if MAX_VALUE > data.get("formingTemperature") > Threshold_forming_temp:
                    ...
                    try:
                        if 851 <= data.get("formingTemperature") <= 1200:
                    ...
                    try:
                        if data.get("O2PressureHeating//") > GL_MAX_PRESSURE_HEAT:
                            GL_MAX_PRESSURE_HEAT = data.get("O2PressureHeating//") / 100
                        if 0 <= data.get("O2PressureHeating//") / 100 <= 7:
                    ...
                    try:
                        if 2 <= data.get("O2PressureCutting") / 100 <= 6:
                    ...
                    try:
                        if data.get("DAAcetylenePressure") > GL_MAX_DAACETYLENE_PRESSURE:
                            GL_MAX_DAACETYLENE_PRESSURE = data.get('DAAcetylenePressure')
                ...
                # elif (time.time() - GL_PREV_CYCL_START) > 110:
                else:
                ...
                if FL_PREV_STATUS != FL_STATUS:
                    ...
                    if not FL_STATUS:
                        ...
                        payload['data'] = {
                            ...
                            # "O2PressureCutting": 0,   #GL_PRESSURE_CUTTING_AVG
