...
class DBHelper:
    def __init__(self):
        ...
        self.cursor.execute("""CREATE TABLE IF NOT EXISTS running_data(timestamp INTEGER, torque REAL,
         angle REAL, serial_number STRING)""")
        self.cursor.execute("""
    ...
    def save_running_data(self, torque, angle, serial_number):
        try:
            self.cursor.execute("""SELECT * FROM running_data where serial_number =?""",
                                (serial_number,))
            data = self.cursor.fetchone()
            if data:
                self.cursor.execute("""UPDATE running_data SET torque = ?, angle = ? WHERE serial_number =?""",
                                    (torque, angle, serial_number))
            else:
                self.cursor.execute("""INSERT INTO running_data(timestamp, angle,  torque,
                serial_number) VALUES(?,?,?,?)""", (time.time(), angle, torque, serial_number))
            self.connection.commit()
