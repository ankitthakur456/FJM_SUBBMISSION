...
def read_values(mb_client, parameters):
    ...
    try:
        ...
        data0 = mb_client.read_holding_registers(1200, 1)
        data1 = mb_client.read_holding_registers(1204, 1)
        data2 = mb_client.read_holding_registers(1207, 1)
        data3 = mb_client.read_holding_registers(1206, 1)
...
def Sending_data_to_1st_server(payload):
    ...
    if GL_SEND_DATA:
        ...
        except Exception as e:  # if publish gives exception
            log.error(f'error is in publishing {e}')
            try:
...
def Sending_data_to_2nd_server(payload):
    ...
    if GL_SEND_DATA:
        ...
        try:
            ...
            log.warning('error in publishing')
            try:
    ...
    while True:
        try:
            if GL_SERIAL_NUMBER:
                ...
                GL_SERIAL_NUMBER = ''
            mb_client1 = initiate_client(GL_IP, GL_U_ID)
            data = read_values(mb_client1, GL_PARAM_LIST)
            if data:
