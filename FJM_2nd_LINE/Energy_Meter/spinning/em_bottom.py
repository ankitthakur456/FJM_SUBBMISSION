...
def get_em_values(unitId, type_):
    ...
    if type_ == '1200':  # DONE
        for i in range(2):
            try:
                register_data = f_list(mb_client.read_registers(2566, 2, 3), True)
            except Exception as e:
                log.error(f"ERROR: {e}")
                register_data = []
                time.sleep(i / 10)
...
def publish_values(payload, topic):
    ...
    if GL_SEND_DATA:
        ...
        try:
            result = ob_client_mqtt.publish(topic, payload_str, qos=2)  # try to publish the data
